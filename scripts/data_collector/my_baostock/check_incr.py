...
if __name__ == "__main__":
  # config
  provider_uri1 = "/capstor/scratch/cscs/ljiayong/datasets/qlib/my_baostock/bin"
  provider_uri2 = "/capstor/scratch/cscs/ljiayong/datasets/qlib/baostock_incremental/bin"
  ...
  scores = []
  for i, provider_uri in enumerate([provider_uri1, provider_uri2]):
    ...
    analysis_position.report_graph(report, show_notebook=False, save_path=os.path.join(os.path.dirname(__file__), f"{market}_{freq}_factor_provider_{i}"))

    import pdb;pdb.set_trace()
  scores[0].eq(scores[1]).all()
