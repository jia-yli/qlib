...
def prepare_indexweight(base_config, T, start_time):
  ...
  qlib.init(**{
    ...
    "exp_manager": {
      ...
      "kwargs": {
        "uri": "file:///" + os.path.join(workspace_path, f"mlrun/prepare_indexweight"),
  ...
  for i in tqdm(range(T - 1, len(price_all))): # T-window, first result is at T-1
    ...
    # save risk data
    root = os.path.join(workspace_path, f"indexweight", date.strftime("%Y%m%d"))
...
if __name__ == "__main__":
  ...
  prepare_indexweight(base_config, T=246, start_time="2021-09-01")
