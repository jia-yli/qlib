...
from qlib.constant import REG_CN, REG_TW, REG_US, REG_CRYPTO
...
CRYPTO_TIME = [datetime.strptime("00:00", "%H:%M"), datetime.strptime("00:00", "%H:%M") + timedelta(days=1)]
...
def get_min_cal(shift: int = 0, region: str = REG_CN) -> List[time]:
    ...
    if region == REG_CN:
        ...
        for ts in list(
            ...
            cal.append(ts.time())
    elif region == REG_CRYPTO:
        for ts in list(
            pd.date_range(CRYPTO_TIME[0], CRYPTO_TIME[1] - timedelta(minutes=1), freq="1min") - pd.Timedelta(minutes=shift)
        ):
            cal.append(ts.time())
    else:
...
def is_single_value(start_time, end_time, freq, region: str = REG_CN):
    ...
    if region == REG_CN:
        ...
        if end_time - start_time < freq:
            return True
        if start_time.hour == 23 and start_time.minute == 59 and start_time.second == 0:
            return True
        return False
...
def time_to_day_index(time_obj: Union[str, datetime], region: str = REG_CN):
    ...
    if region == REG_CN:
        ...
        if CRYPTO_TIME[0] <= time_obj < CRYPTO_TIME[1]:
            return int((time_obj - CRYPTO_TIME[0]).total_seconds() / 60)
        else:
            raise ValueError(f"{time_obj} is not the opening time of the {region} stock market")
    else:
        raise ValueError(f"{region} is not supported")
